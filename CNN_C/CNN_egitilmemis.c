...
ConvLayer* init_conv_layer(int input_c, int filter_count, int kernel_size, int stride, int padding) {
    ...
    for (int i = 0; i < filter_count; i++) {
        ...
        for (int j = 0; j < size; j++) {
            layer->filters[i]->data[j] = ((float)rand() / RAND_MAX * 0.2) - 0.1;
        }
        
        layer->biases[i] = 0.0; // Bias baþlangýçta 0 olsun
    }
    return layer;
}
...
FCLayer* init_fc_layer(int input_size, int output_size) {
    ...
    for (int i = 0; i < input_size * output_size; i++) {
        layer->weights[i] = ((float)rand() / RAND_MAX * 0.02) - 0.01;
    }
...
int main() {
    ...
    // Rastgele birkaç pikseli boyayalým ki veri olsun
    set_vol(input, 14, 14, 0, 1.0); 
    set_vol(input, 15, 14, 0, 1.0);
    printf("[1] Giris Resmi Hazir: 28x28x1\n");
